...
public class FacePredictor {
     ...
     */
    public static List<ROIData> predictFaces(
        byte[] testImgBytes,
    ...
    public static List<ROIData> predictFaces(
        ...
        try {
            ...
            for (int i = 0; i < testRoiMat.rows(); ++i) {
                try {
                    testFeatureMat = ImageUtils.getFeatureMat(
                        faceRecognizer,
                        testImgMat,
                        testRoiMat.row(i)
                    );
     ...
     */
    public static Map<String, List<Mat>> parseKnownFaces(
        String facesDirPath,
        String detectorModelPath,
            ...
            for (File subDir: rootDir.listFiles()) {
                ...
                for (File imgFile: imgFileArr) {
                    try {
                        ...
                        roiMat = FaceDetector.detectFaces(
                            imgMat,
                            detectorModelPath
                        );
                        ...
                        for (int i = 0; i < roiMat.rows(); ++i) {
                            featureMat = ImageUtils.getFeatureMat(
                                faceRecognizer,
                                imgMat,
                                roiMat.row(i)
                            );
